...
⚙️ How It Works
Registration
User opens `afkschool.com` → Route 53 → CloudFront → S3
Parent fills admission form → JavaScript sends POST to API Gateway
Lambda parses data → stores in DynamoDB → success toast shown
Gallery (Admin)
Admin opens `afkschool.com/admin.html`
Enters username + password → OTP sent to Gmail via SES
Enters OTP → verified → admin panel unlocks
Drag & drop photos → Lambda uploads to S3 → metadata saved to DynamoDB
Public visitors see photos at `afkschool.com/gallery.html`
---
🔐 Security
